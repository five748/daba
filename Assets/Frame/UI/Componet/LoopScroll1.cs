...
using UnityEngine;
public class LoopScroll1 : DragSomeOne
{
    public int key = -1;
    ...
    private void InitData(string baseUIName = "")
    {
        ...
        scrollRectWorld += new Vector4(0,0, -200, 200); 
    ...
    public void SetTargetSize()
    {
        ...
        {
            RectRange = new Vector4(0, BeginPos.x, -BeginPos.y - 90, 0);
        }
        FirstItemIndex = 0;
    ...
    private void AddOneItem()
    {
        ...
        TargetEndItemPos += new Vector2(0, size.y);
        size = new Vector2(size.x, -size.y);
        allItemPos.Add(oldPos);
        if (IsOut)
        {
            OutIndex++;
        }
    ...
    {
        Vector2 pos = -GetInRangPos(allItemPos[index]) + modifyBeginPos;
        if (movement == Movement.Horizontal)
        {
