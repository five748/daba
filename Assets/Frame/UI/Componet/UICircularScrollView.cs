...
namespace CircularScrollView
{
    ...
    public class UICircularScrollView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public void ButtonIndexTo(int index,float time = 0)
        {
            if (m_MaxCount < 0)
            {
                return;
            ...
            // * -> 计算每个Cell坐标
            if (index > m_CellInfos.Length)
            {
                index = m_CellInfos.Length;
            }
            var cell = m_CellInfos[index - 1];
            float pos = cell.pos.y;
            if (m_Direction == e_Direction.Vertical)
            {
                var p = -pos - m_ScrollRect.GetComponent<RectTransform>().rect.height + m_CellObjectHeight;
                m_ContentRectTrans.anchoredPosition = new Vector2(m_ContentRectTrans.anchoredPosition.x, Mathf.Max(0,p));
                UpdateCheck();
            }
        }
