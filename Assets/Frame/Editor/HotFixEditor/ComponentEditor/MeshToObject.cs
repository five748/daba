...
public class MeshToObject{
	...
	[MenuItem("GameObject/MeshToObj")]
	public static void MeshAToObject() {
		GameObject[] targets = Selection.gameObjects;
		if(targets.Length != 1)
		{
			return;
		}
		var mf = targets[0].GetComponent<MeshFilter>();
		string path = Application.dataPath + "/test.obj";
		...
		using(StreamWriter streamWriter = new StreamWriter(string.Format("{0}{1}.obj", path, "test")))
		{
			streamWriter.Write(MeshToString(mf, new Vector3(-1f, 1f, 1f)));
			streamWriter.Close();
		}
