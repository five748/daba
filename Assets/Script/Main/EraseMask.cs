...
public class EraseMask : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    [Header("擦拭比例")] public int rate = 35;      //可以设置当到达一定比例后全部显示

    Action eraserEvent;         //擦拭事件
    public RawImage uiTex;             //渲染对象
    ...
    //    Color col = MyTex.GetPixel((int)(localPos.x + mWidth / 2), (int)(localPos.y + mHeight / 2));
    //    if (col.a < 0.1f) {
    //        if (childs != null) {
    //            foreach (var item in childs)
    //            {
    //                Debug.LogError(Vector2.Distance(item.position, worldPos));
    //            }
    //        }
    //    }
    //}
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        modifyPos =  Camera.main.WorldToScreenPoint(beginTran.position) - (Vector3)startPos;
        CheckPoint(startPos);
    }
