namespace Table{
   [System.Serializable]
    public class ad:ITable{
            public int id;
            public string eKey;
            public string des;
            public int reportStart;
            public int reportError;
            public int reportEnd;
            public int Init(string str){
                string[] strs = str.Split('↕');
                if(!string.IsNullOrEmpty(strs[0])){id = int.Parse(strs[0]);}
                if(!string.IsNullOrEmpty(strs[1])){eKey = strs[1];}
                if(!string.IsNullOrEmpty(strs[2])){des = strs[2];}
                if(!string.IsNullOrEmpty(strs[3])){reportStart = int.Parse(strs[3]);}
                if(!string.IsNullOrEmpty(strs[4])){reportError = int.Parse(strs[4]);}
                if(!string.IsNullOrEmpty(strs[5])){reportEnd = int.Parse(strs[5]);}
                return int.Parse(strs[0]);
            }//=======代码自动生成请勿修改=======
        }
    }
